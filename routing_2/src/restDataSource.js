...
export class RestDataSource {
    ...
    // getProducts() {
    //     // - the result from the Axios get method is a Promise that will produce
    //     // the response from the server when the HTTP request has completed.
    //     // - use then method to specify what will happen when the work
    //     // represented by a Promise is complete
    //     return Axios.get(baseUrl);
    // }

    // async/await version without "then" method
    // async/await keywords will allow the component that requires the operation
