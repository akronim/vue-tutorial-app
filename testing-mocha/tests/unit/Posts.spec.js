...
describe('Posts.vue', () => {
  it('renders a message if passed', () => {
    ...
    expect(wrapper.find('#message').text()).to.eq('New content coming soon!')
  })
  ...
  it('renders posts', async () => {
    ...
    expect(wrapper.findAll('.post').length).to.eq(1)
  })
  ...
  it('does not render new post link if not authenticated', async () => {
    ...
    expect(wrapper.find('.new-post').exists()).to.eq(false)
  })
})
